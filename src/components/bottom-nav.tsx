...
export function BottomNav() {
  ...
  return (
    <nav className="glass fixed inset-x-0 bottom-0 z-40 border-t border-white/10 pb-[env(safe-area-inset-bottom)] sm:hidden">
      <div className="mx-auto flex h-16 max-w-md items-center justify-between px-3">
        {ITEMS.map(({ href, label, icon: Icon, ...rest }) => {
          ...
          if (central) {
            return (
              <Link key={href} href={href} aria-label={label} className="-mt-6">
          ...
          return (
            <Link
              ...
              href={href}
              aria-label={label}
              className={`relative flex flex-col items-center gap-0.5 px-2.5 py-2 text-[10px] font-medium transition-colors ${
            ...
            >
              ...
              <Icon className="size-6" strokeWidth={active ? 2.3 : 1.8} />
              {label}
            </Link>
          );
        })}
